...
use common::{
    ...
    redr,
    sha256_utils::{convert_sha256_to_string, sha256_from_vec_of_vec, Sha256Buff},
};
use object::{Import, Object};

use super::{
    signature::{SigData, SigId, Signature},
...
impl HeurSet {
    ...
    pub(crate) const SET_MAGIC_U32: u32 = 0x54453548;
...
impl SigSetTrait for HeurSet {
    fn append_signature(&mut self, sig_id: SigId, sig: Signature) {
        ...
        let name = sig.name();
        let SigData::Imports(imports) = sig.data else {
            todo!();
        };
        // 2 Sigs:
        //      - 0. AAAA, [0. kernel32+sleep, 1. user32.dll+messageboxa]
